...
st.set_page_config(page_title="Phone pe pulse data visualization | By Maxwell",
                   page_icon=icon,
                   layout="wide",
                   )
...
def original_state_list():
    cursor.execute("""select distinct state 
...
def year_list():
    cursor.execute(
        "SELECT distinct year FROM phonepe_pulse.aggregated_transaction order by year asc;")
    data = cursor.fetchall()
...
def quarter_list():
    cursor.execute(
        "SELECT distinct quarter FROM phonepe_pulse.aggregated_transaction order by quarter asc;")
    data = cursor.fetchall()
    data = [i[0] for i in data]
    return data
...
def get_transaction_type():
    cursor.execute(
        "SELECT distinct transaction_type FROM phonepe_pulse.aggregated_transaction;")
    data = cursor.fetchall()
    data = [i[0] for i in data]
    return data
...
def get_map_transaction():
    cursor.execute("SELECT * FROM phonepe_pulse.map_transaction;")
    data = cursor.fetchall()
    d = pd.DataFrame(data, columns=cursor.column_names)
    return d
...
def get_agg_users():
    cursor.execute("SELECT * FROM phonepe_pulse.aggregated_users;")
    data = cursor.fetchall()
    d = pd.DataFrame(data, columns=cursor.column_names)
    return d
...
def get_map_users():
    cursor.execute("SELECT * FROM phonepe_pulse.map_users;")
    data = cursor.fetchall()
    d = pd.DataFrame(data, columns=cursor.column_names)
    return d
    ...
    with st.container():
        ...
        with col1:
            def get_aggregated_user():
                cursor.execute(
                    "SELECT * FROM phonepe_pulse.aggregated_transaction;")
                data = cursor.fetchall()
...
if selected == "Users-Insights":
    ...
    with col1:
        ...
        fig = px.scatter(user, x="User_percentage", y="User_count", title="bubble plot for users brands",
                         size="User_count", color="User_brand", log_x=True, size_max=20)
